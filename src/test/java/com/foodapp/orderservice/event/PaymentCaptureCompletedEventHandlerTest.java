...
import com.foodapp.orderservice.event.consumer.PaymentCaptureCompletedEventHandler;
...
class PaymentCaptureCompletedEventHandlerTest {

    @Mock OrderRepository orderRepository;
    @Spy  OrderStateMachine stateMachine = new OrderStateMachine();
    @Mock OrderEventPublisher eventPublisher;
    @InjectMocks PaymentCaptureCompletedEventHandler handler;

    private final UUID userId = UUID.randomUUID();
    private final UUID restaurantId = UUID.randomUUID();

    @Test
    void shouldTransitionOrderToPaid() {
